...
class FindWoodPredicate(Predicate):
    ...
    def __init__(self, blast_strength, bomb_blast_strength):
...
class PositionNode:
    ...
    def __init__(self, parent, position, action):
        self.parent = parent
    ...
    @staticmethod
    def get_path_length_and_action(node):
...
def bfs(board, start_position, start_actions, predicate):
...
def valid_agent_position(board, pos):
    board_size = len(board)
    r, c = pos
    return 0 <= r < board_size and 0 <= c < board_size and \
        board[r, c] in ACCESSIBLE_TILES


def get_in_range(board, position, blast_strength):
